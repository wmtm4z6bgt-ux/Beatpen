    ...
    <Card className="fixed bottom-6 right-6 w-96 h-[600px] shadow-2xl rounded-2xl bg-black/50 backdrop-blur-lg border-violet-500/20 flex flex-col overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between p-4 border-b border-white/10">
        <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-lg font-semibold text-white">BeatPen Guide</CardTitle>
        </div>
        ...
        <ScrollArea className="h-full">
            ...
            {messages.length === 0 && (
                <div className="text-center text-muted-foreground text-sm p-8">
                    Ask me anything about finding internships or using BeatPen!
                </div>
            )}
            {messages.map(m => (
                ...
                <div className={cn("max-w-xs rounded-2xl px-4 py-2 text-sm", m.role === 'user' ? 'bg-primary text-primary-foreground rounded-br-none' : 'bg-secondary text-secondary-foreground rounded-bl-none')}>
                    {m.content}
                </div>
                 {m.role === 'user' && (
                    <Avatar className="h-8 w-8">
            ...
            {isLoading && (
                <div className="flex gap-3 justify-start">
                     ...
                     <div className="max-w-xs rounded-2xl px-4 py-2 bg-secondary text-secondary-foreground rounded-bl-none flex items-center">
